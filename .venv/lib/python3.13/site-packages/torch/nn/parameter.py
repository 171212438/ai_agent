...
# 这个元类同时具备两类能力：
#   1. 继承 torch.Tensor 原本的 _TensorMeta 能力；
#   2. 额外重写 Parameter 的 isinstance 判断逻辑
#
# 在执行 isinstance(t, cls) 时，当 cls 是 Parameter，或 Parameter 的子类，或其他使用
# _ParameterMeta 的类时，会进入 _ParameterMeta.__instancecheck__，对应关系：
#    self     -> cls
#    instance -> t
class _ParameterMeta(torch._C._TensorMeta):
    # Make `isinstance(t, Parameter)` return True for custom tensor instances that have the _is_param flag.
    def __instancecheck__(self, instance) -> bool:
        if self is Parameter:  # cls 是 Parameter 时，才启用 _is_param 的特殊判断逻辑
            # 如果 instance 是一个 Tensor，且它身上有 _is_param=True，它就是 Parameter
            # 这里会走 type(torch.Tensor).__instancecheck__(torch.Tensor, instance)
            if isinstance(instance, torch.Tensor) and getattr(instance, "_is_param", False):
                return True
        # 不满足特殊条件，就交回父类，做正常的 Tensor / Parameter 类型判断逻辑
        return super().__instancecheck__(instance)


# 定义一个 torch.Tensor 子类，同时使用元类 _ParameterMeta 来控制一些特殊行为
# _ParameterMeta 重写 __instancecheck__：让带有 _is_param=True 标记的自定
# 义 Tensor，在执行 isinstance(t, Parameter) 时也能被识别为 Parameter
class Parameter(torch.Tensor, metaclass=_ParameterMeta):
    ...
    def __new__(cls, data=None, requires_grad=True):
        if data is None:
            data = torch.empty(0)  # 没传入 tensor，则创建一个空 tensor
        if type(data) is torch.Tensor or type(data) is Parameter:
            # For ease of BC maintenance, keep this path for standard Tensor.
            # Eventually (tm), we should change the behavior for standard Tensor to match.
            # 基于已有 data 的底层 Tensor 数据，创建一个 Parameter 子类对象
            return torch.Tensor._make_subclass(cls, data, requires_grad)

        # Path for custom tensors: set a flag on the instance to indicate parameter-ness.
        t = data.detach().requires_grad_(requires_grad)  # data 从原 autograd 计算图里断开，得到新的 leaf tensor，后原地设置该 detached tensor 是否需要梯度
    ...
    # 深拷贝一个新的 Parameter 对象，数据值相同，但底层存储不同，并保留 requires_grad 设置
    def __deepcopy__(self, memo):
        if id(self) in memo:
            return memo[id(self)]  # 已经复制过，直接返回之前的副本
        else:
            result = type(self)(

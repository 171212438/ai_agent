...
def _forward_unimplemented(self, *input: Any) -> None:
    ...
    """
    # 如果一个子类没有自己实现 forward()，调用模型时就会报错
...
class Module:
    ...
    training: bool  # 模型处于训练模式还是推理模式
    _parameters: dict[str, Parameter | None]  # 保存模型参数的字典，self.xxx = Parmeter(...) 会被注册到 self._parameters["xxx"]
    _buffers: dict[str, Tensor | None]  # 模型状态
    _non_persistent_buffers_set: set[str]  # 集合，记录哪些 buffer 不需要保存进 state_dict
    _backward_pre_hooks: dict[int, Callable]  # 反向传播前调用的 hook
    _backward_hooks: dict[int, Callable]  # 反向传播中/后调用的 hook
    _is_full_backward_hook: bool | None  # 标记是否使用 full backward hook
    _forward_hooks: dict[int, Callable]  # 保存 forward 之后执行的 hook
    ...
    _forward_hooks_with_kwargs: dict[int, bool]  # 哪些 forward hook 支持 kwargs
    # forward hooks that should always be called even if an exception is raised
    _forward_hooks_always_called: dict[int, bool]  # 记录哪些 forward hook 即使发生异常也要调用
    _forward_pre_hooks: dict[int, Callable]  # 保存 forward 之前执行的 hook
    # Marks whether the corresponding _forward_hooks accept kwargs or not.
    # As JIT does not support set[int], this dict is used as a set, where all
    # hooks represented in this dict accept kwargs.
    _forward_pre_hooks_with_kwargs: dict[int, bool]  # 哪些 forward 前 hook 支持 kwargs
    _state_dict_hooks: dict[int, Callable]  # state_dict 生成后调用
    _load_state_dict_pre_hooks: dict[int, Callable]  # load_state_dict 加载前调用
    _state_dict_pre_hooks: dict[int, Callable]  # state_dict 生成前调用
    _load_state_dict_post_hooks: dict[int, Callable]  # load_state_dict 加载后调用
    _modules: dict[str, Optional["Module"]]  # noqa: UP045  # 保存子模块的字典
    call_super_init: bool = False  # 控制 Module.__init__() 末尾是否继续调用更上层父类的 __init__
    _compiled_call_impl: Callable | None = None  # 如果有编译优化，则可能走 _compiled_call_impl

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initialize internal Module state, shared by both nn.Module and ScriptModule."""
        torch._C._log_api_usage_once("python.nn_module")  # PyTorch 内部统计/日志用途

        # Backward compatibility: no args used to be allowed when call_super_init=False
        # 如果 call_super_init 是 False，就不允许给 Module.__init__() 传额外参数
        ...
        """
        # 此时内部字典还没建好，所以必须绕开 Module.__setattr__，直接调用更底层的 super().__setattr__(...)
        super().__setattr__("training", True)  # self.training = True，默认训练模式
        super().__setattr__("_parameters", {})  # 创建参数账本，以后 self.xxx = Parameter(...) 都会被注册到 self._parameters["xxx"]
        super().__setattr__("_buffers", {})  # 创建 buffer 账本
        super().__setattr__("_non_persistent_buffers_set", set())  # 记录哪些 buffer 不需要保存进 state_dict()，如果名字出现在这个集合里，就不会持久化保存
        super().__setattr__("_backward_pre_hooks", OrderedDict())  # 初始化各种 hook 容器
        ...
        super().__setattr__("_modules", {})  # 创建子模块账本，self.W_q = nn.Linear(d_model, d_model)，self._modules["W_q"]

        if self.call_super_init:  # 是否继续调用更上层父类
            super().__init__(*args, **kwargs)

    # 给 nn.Module 默认绑定一个 forward 方法占位符
    forward: Callable[..., Any] = _forward_unimplemented  # forward 是一个可以被调用的对象，可以接收任意参数，返回值可以是任意类型

    # 把一个 Tensor 注册成 Module 的 buffer，即把一个 Tensor 添加到当前 Module 的 buffer 管理列表里
    def register_buffer(
        ...
        """
        # 如果当前模块是 ScriptModule，则不支持 non-persistent buffer
        if persistent is False and isinstance(self, torch.jit.ScriptModule):
            raise RuntimeError("ScriptModule does not support non-persistent buffers")

        if "_buffers" not in self.__dict__:
            raise AttributeError("cannot assign buffer before Module.__init__() call")  # 必须先调用 super().__init__()
            ...
            raise KeyError(f"attribute '{name}' already exists")  # 如果这个模块已经有同名属性，而且这个名字之前不是 buffer，不允许注册
        elif tensor is not None and not (
            isinstance(tensor, torch.Tensor) or hasattr(tensor, "__torch_function__")
        ):
            # buffer 必须是 Tensor 或支持 __torch_function__ 的 Tensor-like 对象
            raise TypeError(
                f"cannot assign '{torch.typename(tensor)}' object to buffer '{name}' "
                "(torch Tensor or None required)"
            )
        else:
            # 如果外部注册了 hook，就允许它在 buffer 注册前检查或替换 tensor，平常用不到
            for hook in _global_buffer_registration_hooks.values():
                output = hook(self, name, tensor)
                if output is not None:
                    tensor = output
            self._buffers[name] = tensor  # 真正注册 buffer，self._buffers["running_mean"] = torch.zeros(num_features)
            # 处理是否进入 state_dict
            if persistent:
                self._non_persistent_buffers_set.discard(name)  # remove 有可能报错
            else:
